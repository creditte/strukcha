-- Add unit_holder to the relationship_type enum
ALTER TYPE public.relationship_type ADD VALUE IF NOT EXISTS 'unit_holder';

-- Update the validation trigger to handle unit_holder
...
BEGIN
  -- Skip validation for soft-deletes
  ...
  CASE NEW.relationship_type
    ...
    WHEN 'unit_holder' THEN
      IF NOT (_from_type IN ('Individual', 'Company', 'smsf') OR _is_disc_trust_source OR _from_type = 'trust_unit') THEN
        RAISE EXCEPTION 'Unit holders must be individuals, companies, trusts, or SMSFs.';
      END IF;
      IF _to_type != 'trust_unit' THEN
        RAISE EXCEPTION 'Unit holders can only be linked to unit trusts.';
      END IF;
    ...
    WHEN 'beneficiary' THEN
      -- Bare trusts: only Individual, Company, SMSF allowed as beneficiaries
      IF _to_type = 'trust_bare' THEN
        IF _from_type NOT IN ('Individual', 'Company', 'smsf') THEN
          RAISE EXCEPTION 'Bare trust beneficiaries must be individuals, companies, or SMSFs.';
        END IF;
      ELSE
