REVOKE ALL ON FUNCTION public.tenant_has_unlimited_structures(uuid) FROM PUBLIC, anon;
...
BEGIN
  ...
  SELECT coalesce(diagram_limit, 0), coalesce(diagram_count, 0), coalesce(unlimited_structures, false)
  INTO _limit, _used, _unlimited
  FROM public.tenants WHERE id = _tenant_id;

  IF _enforce AND _limit > 0 AND NOT _unlimited THEN
