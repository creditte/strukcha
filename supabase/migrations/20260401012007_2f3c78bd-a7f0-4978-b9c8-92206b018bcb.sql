...
DROP TRIGGER IF EXISTS trg_validate_director_relationship ON public.relationships;
DROP FUNCTION IF EXISTS public.validate_director_relationship();

-- Create comprehensive validation function matching the central rules engine
...
BEGIN
  ...
  -- If we can't resolve entity types, allow (entity might be mid-creation)
  ...
  CASE NEW.relationship_type
    ...
    WHEN 'member' THEN
      IF NOT (_from_type IN ('Individual', 'Company', 'smsf') OR _is_disc_trust_source) THEN
        RAISE EXCEPTION 'Unitholders can only be linked to unit trusts.';
      END IF;
    ...
    ELSE
      -- Unknown relationship type: allow
      NULL;
  END CASE;
...
CREATE TRIGGER trg_validate_relationship_rules
  BEFORE INSERT OR UPDATE ON public.relationships
  FOR EACH ROW
  EXECUTE FUNCTION public.validate_relationship_rules();
